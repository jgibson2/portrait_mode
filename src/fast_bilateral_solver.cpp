...
void fast_bilateral_solver::splat_initial_image(const bilateral_grid_simplified& grid, const cv::Mat initial_image_float, double* parameters)
{
	...
	for (int i = 0; i < nb_pixels; ++i)
	{
        if(std::isnan(pff[i]))
        {
            //std::cerr << "WARNING: nan encountered in image data!" << std::endl;
            in_vec(i) = 0.0f;
        } else {
            in_vec(i) = pff[i];
        }
	...
	splat_in_vec = splat_in_vec.cwiseQuotient((normalization_weights.array() + 0.0001f).matrix());

	for (int i = 0; i < grid.get_nb_vertices(); ++i)
	{
	    if(std::isnan(splat_in_vec(i)))
        {
	        std::cerr << "WARNING: nan encountered in parameters" << std::endl;
        }
		parameters[i] = splat_in_vec(i);
	}
}

std::optional<cv::Mat> fast_bilateral_solver::solve(const cv::Mat& initial_image_float, const bilateral_grid_simplified& grid, const std::vector<int>& data_cost_lookup, const int disparity_range, const float lambda, const int max_num_iterations, const int keep_nb_of_intermediate_images)
{
	assert(initial_image_float.type() == CV_32FC1);
	...
	auto* pr = new fast_bilateral_problem(mat_C_CBC, grid, data_cost_lookup, disparity_range, lambda, keep_nb_of_intermediate_images);
	...
	return std::optional<cv::Mat>(result_image);
}